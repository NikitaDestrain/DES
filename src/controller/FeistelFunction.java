...
public class FeistelFunction {
    ...
    private static FeistelFunction instance;

    private FeistelFunction() {
    }

    public static FeistelFunction getInstance() {
        if (instance == null) instance = new FeistelFunction();
        return instance;
    }

    private long expansionPermutation(int input) {
        return DESUtils.genericPermutation(input, E, 32);
    }
    ...
    private int PBoxPermutation(int input) {
        return (int) DESUtils.genericPermutation(input, P_BOX, 32);
    }
    ...
    int F(int input, long roundKey) {
        ...
        for (int i = 0; i < 8; i++) {
            SBoxOutputs <<= 4;
            SBoxOutputs |= SBoxSubstitution(i, (byte) ((output & EncryptionConstants.MASK_6_BITS) >> 42));
            output = output << 6;
        }
